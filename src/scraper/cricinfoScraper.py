...
class CricinfoScraper:
    ...
    def switchInnings(self):
        ...
        self.driver.execute_script("window.scrollTo(0, " + str(500) + ")")
        actionChains = ActionChains(self.driver)
        button = self.driver.find_elements(By.CLASS_NAME, 'icon-keyboard_arrow_down-outlined')[0]
        actionChains.move_to_element(button).click().perform()

        time.sleep(1)
        actionChains = ActionChains(self.driver)
        next_innings_button = self.driver.find_element(By.CLASS_NAME, 'ds-dropdown')
